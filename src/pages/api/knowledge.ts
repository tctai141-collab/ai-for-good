...
import { extractKnowledge, MAX_TRANSCRIPT_CHARS } from "../../lib/extract";
...
/** One import is a mentor session, not a library. */
...
export const POST: APIRoute = async ({ cookies, request }) => {
  try {
    ...
    if (body.action === "extract") {
      ...
      try {
        const candidates = await extractKnowledge(transcript, source);
        // Deliberately not recorded as an admin action: nothing changed. The
        // audit log tracks writes, and a read that produces a suggestion is not
        // one until somebody imports it.
        ...
        return json({ error: "That chunk could not be read. Nothing was saved." }, 502);
      }
    }
