/**
 * The F26 Trend Research and Builder Sprint schedule, weeks 1 to 7.
 *
 * Lives in lib rather than in the script because two things load it now: the
 * script, and the button on /admin for anybody who would rather not open a
 * shell. One copy, so the two cannot drift.
 *
 * Two rules were applied to the source plan, both about not telling the cohort
 * something that is not true yet:
 *
 *   A speaker is named only where the plan said CONFIRMED. Everything still at
 *   "email sent" or "requested on LinkedIn" keeps its topic and says the
 *   speaker is not fixed. Publishing a name that then does not turn up is
 ...
 *   weekday; those are in F26_DATE_CONFLICTS and are shown every time this is
 *   loaded, rather than being quietly resolved.
 */

import type { ProgrammeEventRow } from "../db/index";
...
/** Stable, readable, and the same on every run, so a re-import updates. */
...
/** Every entry, ready to upsert. */
export const F26_SCHEDULE: ProgrammeEventRow[] = SEEDS.map((seed) => ({
  id: idFor(seed),
  title: seed.title,
  kind: seed.kind,
  startsOn: seed.date,
...
export const F26_DATE_CONFLICTS = [
