...
/**
 * The survey was removed on 15 September 2026, the day its first round ran,
 * because the answers were sensitive and should not be kept.
 *
 * Drops every survey table. secure_delete zeroes the pages those rows were on
 * as they are freed, and VACUUM rewrites the file, so no fragment of an answer
 * survives in the database file afterwards — deleting rows alone leaves them
 * readable in free pages. Encrypted backups taken before this still hold them
 * and expire on their own schedule.
 *
 * Does nothing on a database that never had these tables, so the VACUUM runs
 * once, on the first boot after the deploy, and not on every start.
 */
const SURVEY_TABLES = [
  "survey_answers", "survey_submissions", "survey_items", "survey_groups", "survey_rounds", "survey_seeds",
];

function purgeSurveys(db: Database) {
  const present = db
    .query(`SELECT name FROM sqlite_master WHERE type = 'table' AND name IN (${SURVEY_TABLES.map((t) => `'${t}'`).join(", ")})`)
    .all() as { name: string }[];
  if (present.length === 0) return;
  db.run("PRAGMA secure_delete = ON");
  for (const table of SURVEY_TABLES) db.run(`DROP TABLE IF EXISTS ${table}`);
  db.run("VACUUM");
  console.info(`[survey] removed ${present.length} survey tables and vacuumed the database`);
}
