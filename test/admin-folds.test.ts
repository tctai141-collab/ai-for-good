 ...
 * People was 2.8 screens and Survey 2.4. Now each of those lives behind a
...
describe("what is folded away", () => {
  test("every add form sits in a fold", () => {
    for (const form of ["add-one", "add-bulk", "add-deadline", "event-form", "programme-form", "bug-form", "add-book", "knowledge-form", "survey-form"]) {
      expect([form, insideFold(form)]).toEqual([form, true]);
    }
  });

  test("the lists people act on do not", () => {
    for (const list of ["users", "deadline-list", "event-list", "programme-list", "wish-list", "bug-list", "survey-table", "library-list", "shared-list", "knowledge-list"]) {
...
describe("nothing becomes unreachable", () => {
  ...
  test("Edit buttons that load a form in place open its fold first", () => {
    ...
    expect(script).toContain('reveal(document.getElementById("sv-title"));');
  });
